...
const getAllCourses = (req, res) => {
  try {
    ...
    let response = coursesService.getAllCourses();

    if (nombreCurso.trim()) {
      const aux = nombreCurso.toLowerCase();
      response = response.filter(c =>
        c.nombre.toLowerCase().includes(aux)
      );
    }

    if (nombreProfesor.trim()) {
      const aux = nombreProfesor.toLowerCase();
      response = response.filter(c =>
        c.profesor.toLowerCase().includes(aux)
      );
    }

    isApi(req) ? res.status(200).send(response)
      : res.render("courses/list", { response, filtros: { nombreCurso, nombreProfesor } });
  } catch (error) {
    res.status(500).send("No se pudieron obtener los cursos");
  }
};
